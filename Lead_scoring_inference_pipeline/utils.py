...
def encode_features():
    ...
    print('completed encode_features step')
    conn.close()
...
def get_models_prediction():
    ...
    #loaded_model = mlflow.pyfunc.load_model(model_uri=f"models:/{EXPERIMENT}/{MODEL_NAME}/{STAGE}")
    ...
    print('completed get_models_prediction step')
    conn.close()
...
def prediction_ratio_check():
    ...
    output_df = df['app_complete_flag'].value_counts(normalize=True).reset_index()
    key_value_pairs_str = "\n".join(f"{key} = {value}" for key, value in zip(output_df['index'], output_df['app_complete_flag']))

    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    key_value_pairs_str += f"\nTimestamp: {timestamp}"

    file_path = FILE_PATH

    with open(file_path, 'w') as file:
        file.write(key_value_pairs_str)
        
    print('completed prediction_ratio_check step')
    conn.close()
...
def input_features_check():
    ...
    if sorted(df.columns) == sorted(ONE_HOT_ENCODED_FEATURES):
        print('All the models input are present')
    else:
        print('Some of the models inputs are missing')
    print('completed input_features_check step')
    conn.close()
